...
Panels are selected from format headings and printed column-number sequences in the PDF text layer. Column centers come from those printed numbers. The anchor identity columns determine row bands; continuation panels are matched monotonically or projected affinely when the scan is too faint. Deskew applies the same transform to raster and embedded-word coordinates. Schemas may opt into hierarchical rows. `format_002` does so for MedEdu: town-level parents stay aligned across panels, while printed sub-rows in columns 3–4 are expanded before normalization. Civic and Tahsil retain their flat row contract.
