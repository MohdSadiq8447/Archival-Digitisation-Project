...
from extract_tamil_nadu_tables import PAGE_MAP, REVIEW_RECORDS, ROOT, SOURCE_DIR, OUTPUT_DIR, box_tuple, output_path
...
def main() -> None:
    ...
    manifest = json.loads((OUTPUT_DIR / "1971_Tamil_Nadu_manifest.json").read_text(encoding="utf-8"))
    review = json.loads((OUTPUT_DIR / "1971_Tamil_Nadu_review.json").read_text(encoding="utf-8"))
    if len(manifest) != 27:
        raise ValueError(f"expected 27 manifest entries, found {len(manifest)}")
    ...
    for item in manifest:
        ...
        for source_page_number, target_page in zip(expected_pages, target_reader.pages):
            ...
            if not (target_page.extract_text() or "").strip():
                raise ValueError(f"{Path(item['output']).name}: page has no readable text")
    ...
    print(f"Verified {len(manifest)} Tamil Nadu PDFs, {sum(item['output_pages'] for item in manifest)} pages, source geometry, rotation, content, manifest, and review records")
