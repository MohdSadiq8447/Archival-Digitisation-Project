...
SCRATCH = Path(r"C:\Users\USER\.codex\visualizations\2026\09\07\01a07b9c-e77d-7511-9f66-aafc92441f90\arunachal-output-renders")
...
def main() -> None:
    ...
    for district, district_pdfs in sorted(by_district.items()):
        ...
        for pdf in district_pdfs:
            subprocess.run(
                [str(POPPLER), "-jpeg", "-r", "110", str(pdf), str(district_dir / pdf.stem)],
                check=True,
        ...
        cols = 2
        cell_w, cell_h = 450, 600
        rows = (len(images) + cols - 1) // cols
        sheet = Image.new("RGB", (cols * cell_w, max(1, rows) * cell_h), "white")
        draw = ImageDraw.Draw(sheet)
        for index, (label, image) in enumerate(images):
            col, row = index % cols, index // cols
            x, y = col * cell_w + 12, row * cell_h + 28
            draw.text((x, row * cell_h + 7), label, fill="black")
            sheet.paste(image, (x, y))
        sheet.save(SCRATCH / f"{district.replace(' ', '_')}_contact.jpg", quality=90)

    print(f"Rendered {len(pdfs)} Arunachal Pradesh output PDFs to {SCRATCH}")
