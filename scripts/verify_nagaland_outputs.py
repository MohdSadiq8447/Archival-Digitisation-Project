...
from extract_nagaland_tables import PAGE_MAP, REVIEW_RECORDS, ROOT, SOURCE_DIR, OUTPUT_DIR, output_path, box_tuple
...
def main() -> None:
    ...
    manifest = json.loads((OUTPUT_DIR / "1971_Nagaland_manifest.json").read_text(encoding="utf-8"))
    review = json.loads((OUTPUT_DIR / "1971_Nagaland_review.json").read_text(encoding="utf-8"))
    if len(manifest) != 4:
        raise ValueError(f"expected 4 manifest entries, found {len(manifest)}")
    ...
    print(f"Verified {len(manifest)} Nagaland PDFs, {sum(item['output_pages'] for item in manifest)} pages, source geometry, rotation, content, manifest, and review records")
