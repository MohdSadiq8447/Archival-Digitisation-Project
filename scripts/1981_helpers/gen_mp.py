...
# CIVIC
civic_pdfs = []
for fn in os.listdir('Madhya Pradesh/civic amenities'):
...
# MEDEDU
med_pdfs = []
for fn in os.listdir('Madhya Pradesh/mededu'):
    if not fn.endswith('.pdf'): continue
    # Standard 2-page spread for MP
...
# TEHSIL
teh_pdfs = []
if os.path.exists('Madhya Pradesh/tehsil appendix'):
    for fn in os.listdir('Madhya Pradesh/tehsil appendix'):
        if not fn.endswith('.pdf'): continue
        # Standard 4-page spread for MP
        pag = {
            'page_1_columns': [1, 17],
            'page_2_columns': [18, 34],
...
os.makedirs('Madhya_Pradesh_Formats', exist_ok=True)
with open('Madhya_Pradesh_Formats/pdf_formats.yaml', 'w') as f:
