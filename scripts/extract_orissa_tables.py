...
SOURCE_DIR = ROOT / "1971" / "Orissa"
OUTPUT_DIR = ROOT / "output" / "pdf" / "1971_trimmed" / "Orissa"

PAGE_MAP = {
    "1971 Puri.pdf": {
        "civic_amenities": [26],
        "medical_educational_amenities": [27],
        "tehsil_appendix": [547, 548],
    }
}
...
def main() -> None:
    ...
    if {path.name for path in source_paths} != set(PAGE_MAP):
        raise ValueError("1971 Orissa source inventory mismatch")
    ...
    if len(manifest) != 3:
        raise ValueError(f"expected 3 Orissa outputs, generated {len(manifest)}")
    (OUTPUT_DIR / "1971_Orissa_manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    (OUTPUT_DIR / "1971_Orissa_review.json").write_text(json.dumps(REVIEW_RECORDS, indent=2), encoding="utf-8")
    print(f"Generated and reopened/verified {len(manifest)} Orissa PDFs in {OUTPUT_DIR}")
