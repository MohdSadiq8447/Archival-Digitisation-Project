...
from extract_sikkim_tables import PAGE_MAP, REVIEW_RECORDS, ROOT, SOURCE_DIR, OUTPUT_DIR, box_tuple, output_path


def main() -> None:
    source_names = {path.name for path in SOURCE_DIR.glob("1971 *.pdf")}
    if source_names != set(PAGE_MAP):
        raise ValueError("Sikkim source inventory mismatch")
    manifest = json.loads((OUTPUT_DIR / "1971_Sikkim_manifest.json").read_text(encoding="utf-8"))
    review = json.loads((OUTPUT_DIR / "1971_Sikkim_review.json").read_text(encoding="utf-8"))
    if len(manifest) != 2 or review != REVIEW_RECORDS:
        raise ValueError("Sikkim manifest/review record mismatch")
    seen = set()
    ...
    if seen != expected_seen:
        raise ValueError("Sikkim manifest key mismatch")
    ...
    print(f"Verified {len(manifest)} Sikkim PDFs, {sum(item['output_pages'] for item in manifest)} pages, source geometry, rotation, content, manifest, and review records")
