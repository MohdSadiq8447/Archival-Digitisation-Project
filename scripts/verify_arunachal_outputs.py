...
from extract_arunachal_tables import DISTRICTS, OUTPUT_DIR, ROOT, SOURCE_DIR, TABLE_SUFFIX, REVIEW_RECORDS, box_tuple, output_path


def main() -> None:
    manifest = json.loads((OUTPUT_DIR / "1971_Arunachal_Pradesh_manifest.json").read_text(encoding="utf-8"))
    review = json.loads((OUTPUT_DIR / "1971_Arunachal_Pradesh_review.json").read_text(encoding="utf-8"))
    expected_manifest_count = sum(
        len([table for table in spec if table != "source"])
        for spec in DISTRICTS.values()
    )
    if len(manifest) != expected_manifest_count or review != REVIEW_RECORDS:
        raise ValueError("Arunachal Pradesh manifest/review count or records mismatch")
    ...
    expected_pairs = {(district, table) for district in DISTRICTS for table in DISTRICTS[district] if table != "source"}
    ...
    print(
        f"Verified {len(manifest)} Arunachal Pradesh PDFs, "
