...
CACHE_DIR = Path(
    r"C:\Users\USER\.codex\visualizations\2026\09\07\01a07b9c-e77d-7511-9f66-aafc92441f90\gujarat-locate"
)
...
def extract_text(source_path: Path) -> str:
    ...
    with tempfile.TemporaryDirectory(prefix="gujarat-pdftext-") as temp_dir:
...
def locate(source_path: Path) -> dict[str, object]:
    ...
    for index, line in enumerate(toc_lines):
        ...
        if re.search(r"\biv\b", lower) and "civic" in lower:
            civic_range = page_range_after_anchor(window, r"\biv\b\s+.*?civic") or civic_range
        if re.search(r"\bv\b", lower) and "educational" in lower:
            medical_range = page_range_after_anchor(window, r"\bv\b\s+.*?educational") or medical_range
    ...
    for index, page in enumerate(pages[20:], start=20):
        ...
        appendix_heading = (
            ("talukwise" in lower or "taluka" in lower or "tahsil" in lower or "tehsil" in lower)
    ...
    return {
        ...
        "offset": offset,
        "civic_source": [page + offset for page in range(civic_range[0], civic_range[1] + 1)],
        "medical_source": [page + offset for page in range(medical_range[0], medical_range[1] + 1)],
        "appendix_source": [
