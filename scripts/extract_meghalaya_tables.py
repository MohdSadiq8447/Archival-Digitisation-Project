...
SOURCE_DIR = ROOT / "1971" / "Meghalaya"
OUTPUT_DIR = ROOT / "output" / "pdf" / "1971_trimmed" / "Meghalaya"


PAGE_MAP: dict[str, dict[str, list[int]]] = {
    "1971 U.K. & J. Hills.pdf": {
        "civic_amenities": [212],
        "medical_educational_amenities": [213],
...
REVIEW_RECORDS = [
    {
        "source": "1971 Garo Hills.pdf",
        "table": table,
        "status": "review",
        "reason": "This is a Part X-C full-count census-tables volume, not the Part X-A Village and Town Directory volume; no relevant Statement IV, Statement V, or tehsil/taluk amenities abstract is present.",
    }
...
def extract_one(source_path: Path, district: str, table_key: str, page_numbers: list[int]) -> dict[str, object]:
    ...
    return {"district": district, "table": table_key, "source": str(source_path.relative_to(ROOT)), "output": str(target.relative_to(ROOT)), "source_pages": page_numbers, "output_pages": len(reopened.pages), "page_sizes": page_sizes}


def main() -> None:
    source_paths = sorted(SOURCE_DIR.glob("1971 *.pdf"), key=lambda path: path.name.lower())
    if {path.name for path in source_paths} != set(PAGE_MAP) | {"1971 Garo Hills.pdf"}:
        raise ValueError("1971 Meghalaya source inventory mismatch")
    ...
    if len(manifest) != 3:
        raise ValueError(f"expected 3 Meghalaya outputs, generated {len(manifest)}")
    manifest_path = OUTPUT_DIR / "1971_Meghalaya_manifest.json"
    manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    review_path = OUTPUT_DIR / "1971_Meghalaya_review.json"
    review_path.write_text(json.dumps(REVIEW_RECORDS, indent=2), encoding="utf-8")
    print(f"Generated and reopened/verified {len(manifest)} Meghalaya PDFs in {OUTPUT_DIR}")
    print(manifest_path)
    print(review_path)
